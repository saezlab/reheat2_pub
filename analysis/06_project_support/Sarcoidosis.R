...
projection_table <- "./results/predict_project/HCA_reference/Sarcoidosis.csv"
projection_table_2join <- "./results/predict_project/HCA_reference/Sarcoidosis_factor_coords.csv"
...
# New data
all_data <- readRDS("./data/val_studies/misc/Sarcoidosis_pb.rds")
...
# Meta data
meta_data <- coldat[, c("sample", "disease_state")] %>%
  unique() %>%
  as_tibble()
...
test_projection <- test_projection %>%
  ...
  left_join(meta_data) %>%
  dplyr::rename("study_label" = "disease_state")
