...
p.race<- meta_data$target_sc%>%
  #filter(genetic_HF == "yes")%>%
  #distinct(genetic_HF, study)%>%
  group_by(race, study)%>%count()%>%
  group_by(study)%>%
  mutate(prop=  n / sum(n))%>%
  ggplot(aes(x= study, y = prop, fill = race))+
  geom_col(color="white")+
...
p.supp<- plot_grid(ncol =1,
          ...
          plot_grid(p.age+
                    ...
                    ncol = 2, rel_widths = c(1,0.5),
                    labels = c("A", "B")),
  plot_grid(p.etio_per_study,
            p.sample_acc_study, ncol= 2, 
            rel_widths= c(0.7,1),
            labels = c("C", "D")),
  plot_grid(  p.genetic_per_study,  p.race,
              ncol=2,
              labels = c("E", "F")),
...
pdf("output/figures/meta_data_presentation/meta_composite_supp2.pdf", 
    width= 10, height= 15)
p.supp
dev.off()
