...
source("~/R-projects/HF_meta-analysis/src/data_utils.R") #general functions 
source("~/R-projects/HF_meta-analysis/src/misc_utils.R")
METAheart = readRDS(file = "output/reheat1/METAheart2023.rds") #main object
# METAheart$Magnet <- c()
# METAheart$Hua19<- c()
# METAheart$Flam19$HF_limma
# METAheart$Wang22$HF_limma
library('biomaRt')
library(WriteXLS)
library(fgsea)
library(tidyverse)
...
saveRDS(meta_targets, 
        file = "output/reheat1/figure_objects/meta_targets.rds")
...
write.table(shiny_allgenes,col.names = T,
            ...
            file = "output/reheat1/shiny/all_summary_stats.txt")

saveRDS(fisher_rank, 
        file = "output/reheat1/shiny/fisher_rank.rds")
...
saveRDS(contribution, file = "output/reheat1/figure_objects/contribution.rds")
...
fisher_summary = tibble("gene" = genes,
  ...
  left_join(grch38%>% dplyr::distinct(symbol, description), by = c("gene"="symbol")) %>%
  dplyr::select(gene, description, fisher_pvalue,mean_t,
                mean_lfc)%>%
  filter(!duplicated(gene)) # the join on gene descriptions duplicates some genes

write.table(fisher_summary,col.names = T,
            row.names = F,quote = F,
            sep = "\t",
            file = "output/reheat1/meta_analysis_summary.txt")
