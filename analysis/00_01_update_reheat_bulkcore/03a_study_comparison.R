...
# METAheart$Flam19 <- METAheart$Magnet
# METAheart <- METAheart[names(METAheart) != "Magnet"]
#METAheart%>% saveRDS(file = "output/reheat1/METAheart2023.rds") 
new_studies <- c("Wang22", "Forte22", "Rao21", "Flam19", "Hua19")
...
saveRDS(jaccard_df, file = "output/reheat1/figure_objects/jaccard_df.rds")
...
res_df%>% filter(!Var1 %in% new_studies)%>%
  pull(value)%>% mean()
...
saveRDS(pairwise_200[,c("PredictorExperiment", 
                        "PredictedExperiment", 
                        "single")], file = "output/reheat1/figure_objects/pairwise_200.rds")
